...
namespace MyTrade {
    ...
    public:
        ...
        static void UpdateFlow(std::unordered_map<std::string, cwMarketDataPtr> code2data, std::unordered_map<std::string, cwMarketDataPtr> curPos);// 记录最新持仓状况（方向，数量，成本价格，开仓成本，数量）
        ...
        static std::vector<cwOrderPtr> HandBar(std::unordered_map<std::string, cwMarketDataPtr> code2data/*昨仓数据*/, std::unordered_map<std::string, cwPositionPtr> curPos);
